...
public extension MoyaProvider {
    ...
    public final class func DefaultEndpointMapping(target: Target) -> Endpoint<Target> {
        var url = target.baseURL.absoluteString.stringByAppendingString(target.path)
        //方法返回一个合法的URL字符串   NSCharacterSet.URLQueryAllowedCharacterSet()为url可用的字符集如？之类的  这样就不会转换其中对应的可用字符集
        let newUrl = url.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.URLQueryAllowedCharacterSet())
        switch target.headerType {
        case "LiDongXu":
            return Endpoint(URL: newUrl!, sampleResponseClosure: {.NetworkResponse(200, target.sampleData)}, method: target.method, parameters: target.parameters, parameterEncoding: .URL, httpHeaderFields: nil)
        case "ZhangMingYang":
            return Endpoint(URL: newUrl!, sampleResponseClosure: {.NetworkResponse(200, target.sampleData)}, method: target.method, parameters: target.parameters, parameterEncoding: .JSON, httpHeaderFields: ["Content-Type":"application/json;charset=UTF-8"])
        case "Get":
            return Endpoint(URL: newUrl!, sampleResponseClosure: {.NetworkResponse(200, target.sampleData)}, method: target.method, parameters: target.parameters)
        default:
            return Endpoint(URL: newUrl!, sampleResponseClosure: {.NetworkResponse(200, target.sampleData)}, method: target.method, parameters: target.parameters, parameterEncoding: .JSON, httpHeaderFields: ["Content-Type":"application/json;charset=UTF-8"])
        }
